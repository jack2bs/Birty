...
#define NUM_OF_DIST_SENSORS			8		// Number of distance sensors
#define NUM_OF_OTHER_SENSORS		2		// Number of other sensors
#define NUM_OF_TOTAL_SENSORS (NUM_OF_DIST_SENSORS + NUM_OF_OTHER_SENSORS)	// Total number of sensors


/*g_sensors API*/
uint8_t * getSensorDataPtr(int index);				// Returns a pointer to the lastData array for the sensors
int getSensorData(int index);						// Returns the value of lastData received by the sensor combined into an int

uint8_t * getSensorShouldReadPtr(int index);
int getSensorShouldRead(int index);					// Returns whether the sensor should be read from

int getSensorDeviceAddress(int index);				// Returns the device address for the sensor

void setSensorDataArr(int index, uint8_t * data);	// Sets the sensor data using an array
void setSensorData(int index, int data);			// Sets the sensor data using an int variable

void setSensorShouldRead(int index, int data);		// Set the shouldRead variable
void clearSensorShouldRead(int index);				// Clear the shouldRead variable

void setSensorDeviceAddress(int index, int address);// Sets the sensor's device address
